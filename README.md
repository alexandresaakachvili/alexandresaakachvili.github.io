...
- **Cartes** : pas de suivi du doigt ni de gyroscope — la carte s'enfonce légèrement sous le doigt (`html.is-coarse .card:active`, même chose pour les jeux préférés et la fiche technique) et revient d'un ressort. C'est le seul retour tactile, volontairement.
...
| `clicks7` / `clicks15` / `clicks22` | clics en moins de 2 s | `initLevel()` |
...
Les succès marqués `pc: true` (niveau, clics) n'apparaissent pas sur tactile, où le curseur n'existe pas. Indices et noms ont leur traduction anglaise dans `i18n.js` (`ach.hint.*`, `ach.name.*`) ; un nom sans clé reste tel quel — c'est le cas des jeux de mots déjà en anglais (« Trust or Shoot ? », « World Builder »…).
...
| Sniper | 3 | curseur rond : point, carré, ornements et particules en cercles (`html[data-cursor-shape="round"]`) |
| Taggeur | 1 | la photo du profil est remplacée par sa version taguée (`portrait-tag.jpg` / `portrait-tag-sm.jpg` : moustache et cache-œil) |
| Clé de cuivre / de jade / de cristal | 5 | une clé — le nom suit le thème équipé (`KEY_NAMES` : Vanilla → cuivre, Écolo → jade, Type psy → cristal ; « Copper / Jade / Crystal key » en anglais), le titre se termine par une icône de clé à la taille du texte (`ICONS.key`, `.shop-item__key`), la description dit « Autant symbole de possibilité que de mystère ». L'achat fait glitcher l'écran un instant et débloque le succès « Capitaliste ! » |
...
| Pimp my ride | 3 | autre sprite, flamme de réacteur, tirs ambre avec traînée — verrouillé idem |
| Free hug | 10 | le vaisseau part avec 3 cœurs — **verrouillé** tant que le mini-jeu n'a pas été lancé une fois (`played` dans `as-ach`, posé à l'ouverture de l'arène). Le libellé du verrou dit volontairement « Finir la phase 1 pour débloquer » : c'est un leurre, pour ne pas révéler qu'un mini-jeu existe |
| Mode combat activé | 8 | décharge de clic à 4 dégâts, explosion de charge à 10 — verrouillé idem |
...
**Bonus** (14 % de chance par ennemi tué, 28 % pour les fonceurs, à ramasser en passant dessus) : `↑` balle +1 (jusqu'à 5 en parallèle, ne tombe plus au maximum), un cœur en pixels (ne tombe plus à pleine vie), un écusson = bouclier 3 s, une bombe à tête de mort (tue tout, 10 dégâts au boss), la pièce « A » (ajoutée à la réserve, clé `coins` dans `as-ach`), `»` cadence +1 (5 crans, de 2,5 à 5,5 coups/s).
